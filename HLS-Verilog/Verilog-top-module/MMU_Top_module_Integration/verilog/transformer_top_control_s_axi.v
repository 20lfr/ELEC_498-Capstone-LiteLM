    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CTRL_MEM_DATA_56: begin
                    rdata <= int_ctrl_mem[1823:1792];
                end
                ADDR_CTRL_MEM_DATA_57: begin
                    rdata <= int_ctrl_mem[1855:1824];
                end
                ADDR_CTRL_MEM_DATA_58: begin
                    rdata <= int_ctrl_mem[1887:1856];
                end
                ADDR_CTRL_MEM_DATA_59: begin
                    rdata <= int_ctrl_mem[1919:1888];
                end
                ADDR_CTRL_MEM_DATA_60: begin
                    rdata <= int_ctrl_mem[1951:1920];
                end
                ADDR_CTRL_MEM_DATA_61: begin
                    rdata <= int_ctrl_mem[1983:1952];
                end
                ADDR_CTRL_MEM_DATA_62: begin
                    rdata <= int_ctrl_mem[2015:1984];
                end
                ADDR_CTRL_MEM_DATA_63: begin
                    rdata <= int_ctrl_mem[2047:2016];
                end
                ADDR_CTRL_MEM_DATA_64: begin
                    rdata <= int_ctrl_mem[2079:2048];
                end
                ADDR_CTRL_MEM_DATA_65: begin
                    rdata <= int_ctrl_mem[2111:2080];
                end
                ADDR_CTRL_MEM_DATA_66: begin
                    rdata <= int_ctrl_mem[2143:2112];
                end
...
// int_ctrl_mem[1823:1792]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1823:1792] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_56)
            int_ctrl_mem[1823:1792] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1823:1792] & ~wmask);
    end
end

// int_ctrl_mem[1855:1824]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1855:1824] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_57)
            int_ctrl_mem[1855:1824] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1855:1824] & ~wmask);
    end
end

// int_ctrl_mem[1887:1856]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1887:1856] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_58)
            int_ctrl_mem[1887:1856] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1887:1856] & ~wmask);
    end
end

// int_ctrl_mem[1919:1888]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1919:1888] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_59)
            int_ctrl_mem[1919:1888] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1919:1888] & ~wmask);
    end
end

// int_ctrl_mem[1951:1920]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1951:1920] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_60)
            int_ctrl_mem[1951:1920] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1951:1920] & ~wmask);
    end
end

// int_ctrl_mem[1983:1952]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[1983:1952] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_61)
            int_ctrl_mem[1983:1952] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[1983:1952] & ~wmask);
    end
end

// int_ctrl_mem[2015:1984]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2015:1984] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_62)
            int_ctrl_mem[2015:1984] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[2015:1984] & ~wmask);
    end
end

// int_ctrl_mem[2047:2016]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2047:2016] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_63)
            int_ctrl_mem[2047:2016] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[2047:2016] & ~wmask);
    end
end

// int_ctrl_mem[2079:2048]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2079:2048] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_64)
            int_ctrl_mem[2079:2048] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[2079:2048] & ~wmask);
    end
end

// int_ctrl_mem[2111:2080]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2111:2080] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_65)
            int_ctrl_mem[2111:2080] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[2111:2080] & ~wmask);
    end
end

// int_ctrl_mem[2143:2112]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2143:2112] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CTRL_MEM_DATA_66)
            int_ctrl_mem[2143:2112] <= (WDATA[31:0] & wmask) | (int_ctrl_mem[2143:2112] & ~wmask);
    end
end

// int_ctrl_mem[2175:2144]
always @(posedge ACLK) begin
    if (ARESET)
        int_ctrl_mem[2175:2144] <= 0;
    else if (ACLK_EN) begin
