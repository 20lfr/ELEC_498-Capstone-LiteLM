...
module headed_compute_controller_tb;
  ...
  logic [31:0] mem_op_lat;

  typedef struct packed {
    logic mac_start;       // [76]
    logic capture_pending; // [75]
    logic clear_pending;   // [74]
    logic mac_complete;    // [73]
    logic mac_ready;       // [72]
    logic [7:0] tile;      // [71:64]
    logic [7:0] head;      // [63:56]
    logic [7:0] layer;     // [55:48]
    logic [7:0] op;        // [47:40]
    logic [31:0] instruction; // [39:8]
    logic [7:0] state;        // [7:0]
  } ctx_t;

  ctx_t ctx_i_s;
  ctx_t ctx_o_s;
  assign ctx_i = ctx_i_s;
  assign ctx_o_s = ctx_o;

  localparam int CTX_STATE_LSB = 0;
  localparam int CTX_INSTR_LSB = 8;
  localparam int CTX_OP_LSB = 40;
  localparam int CTX_LAYER_LSB = 48;
  localparam int CTX_HEAD_LSB = 56;
  localparam int CTX_TILE_LSB = 64;
  localparam int CTX_MAC_READY_BIT = 72;
  localparam int CTX_MAC_COMPLETE_BIT = 73;
  localparam int CTX_CLEAR_PENDING_BIT = 74;
  localparam int CTX_CAPTURE_PENDING_BIT = 75;
  localparam int CTX_MAC_START_BIT = 76;
  ...
  always_ff @(posedge ap_clk) begin : ctx_state_latch
    if (ap_rst) begin
      ctx_i_s <= '0;
      ctx_i_s.mac_ready <= 1'b1;
    end else begin
  ...
  always_ff @(posedge ap_clk) begin : mem_op_latch
    if (ap_rst) begin
      mem_op_lat <= 32'd0;
    end else if (mem_op_ap_vld) begin
      mem_op_lat <= mem_op;
    end
  end
  ...
  always_ff @(posedge ap_clk) begin : MEM_controller_respond
    if (ap_rst) begin
      ...
      if (mem_busy) begin
        ...
        if (mem_read_request_ap_vld && mem_read_request) begin
          mem_busy <= 1'b1;
          mem_timer <= MEM_LAT - 1;
          mem_pending <= MEM_READ;
        end else if (mem_write_request_ap_vld && mem_write_request) begin
          mem_busy <= 1'b1;
          mem_timer <= MEM_LAT - 1;
  ...
  always_ff @(posedge ap_clk) begin : TB_monitoring_compute_done
    ...
    if (compute_start && compute_ready_ap_vld && !compute_ready) begin
      compute_start <= 1'b0;
    end
  end
  ...
  always_ff @(posedge ap_clk) begin : TB_FSM_controling_compute
    case (compute_state)
      ...
      OP_SEND: begin
        reset <= 0;
        compute_start <= 1'b0;
        if (compute_ready_ap_vld && compute_ready) begin
          compute_start <= 1'b1;
