...
public:
                           ...
                           int offset, int len)
    {
        ...
        while (remaining > 0) {
            ...
            int burst_len = (remaining > WT_MAX_BURST_LEN) ? WT_MAX_BURST_LEN : remaining;

            read_loop:
            for (int i = 0; i < burst_len; i++) {
                #pragma HLS PIPELINE II=1
                local_buf[current_offset + i] = src[offset + current_offset + i];
            }
                            ...
                            int offset, int len)
    {
        ...
        while (remaining > 0) {
            ...
            for (int i = 0; i < burst_len; i++) {
                #pragma HLS PIPELINE II=1
                dst[offset + current_offset + i] = local_buf[current_offset + i];
            }
