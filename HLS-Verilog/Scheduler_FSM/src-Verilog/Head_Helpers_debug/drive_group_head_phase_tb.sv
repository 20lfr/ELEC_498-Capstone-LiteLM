...
module drive_group_head_phase_tb;
  ...
  logic        ap_idle;
  logic        ap_ready;
  logic [HEAD_CTX_W-1:0] head_ctx_ref_i;
  logic [HEAD_CTX_W-1:0] head_ctx_ref_o;
  ...
  head_ctx_t head_ctx_ref_struct;
  ...
  always_ff @(posedge clk) begin
    if (!ap_rst) begin
      if ($isunknown(head_ctx_ref_i)) begin
        $error("head_ctx_ref_i went X at time %0t", $time);
      end else if (head_ctx_ref_i[63:32] !== base_head_idx) begin
        $error("head head_idx mismatch: saw %0d expected %0d at time %0t", head_ctx_ref_i[63:32], base_head_idx, $time);
      end
    end
  end
  ...
  // ========================================================================
  assign head_ctx_ref_struct = head_ctx_ref_o;
  ...
  always_ff @(posedge clk) begin
    if (ap_rst) begin
      ...
      if (head_ctx_ref_o_ap_vld && gb < HEADS_TOTAL) begin
        head_ctx_shadow[gb] <= head_ctx_ref_struct;
      end
    end
  end
      ...
      always_ff @(posedge clk) begin
        if (ap_rst) begin
          ...
          if (h == base_head_idx && head_ctx_ref_o_ap_vld) begin
            compute_start_now = head_ctx_ref_struct.compute_start;
            dma_start_now     = head_ctx_ref_struct.wl_start;
          end
