...
void init_inputs(
    ...
    int8_t full_weights[D_MODEL * D_MODEL],
    ...
    for (int out = 0; out < D_MODEL; ++out) {
        for (int i = 0; i < D_MODEL; ++i) {
            full_weights[out * D_MODEL + i] = static_cast<int8_t>(7);
        }
    }
}
...
void expected_out_proj_full(
    ...
    const int8_t full_weights[D_MODEL * D_MODEL],
    ...
    for (int out = 0; out < D_MODEL; ++out) {
        int32_t acc = 0;
        for (int i = 0; i < D_MODEL; ++i) {
            const int8_t w = full_weights[out * D_MODEL + i];
...
void load_tile_weights(
    int8_t tile_weights[D_MODEL * D_TILE_WO],
    const int8_t full_weights[D_MODEL * D_MODEL],
    int tile_idx
) {
...
void load_tile_weights_w1(
    int8_t tile_weights[D_MODEL * D_TILE_W1],
    const int8_t full_weights[D_MODEL * D_MODEL],
    int tile_idx
) {
...
void load_tile_weights_w2(
    int8_t tile_weights[D_FFN * D_TILE_W2],
    const int8_t full_weights[D_FFN * D_FFN],
    int tile_idx
) {
...
void print_full_weights(const int8_t full_weights[D_MODEL * D_MODEL]) {
...
int main() {
    ...
    int8_t valueB_in[D_MODEL * D_TILE_WO] = {};
    int8_t full_weights[D_MODEL * D_MODEL] = {};
    int32_t full_bias[D_MODEL] = {};
    int8_t ffn1_weights[D_MODEL * D_TILE_W1] = {};
    int16_t ffn1_out[D_TILE_W1] = {};
    int16_t ffn1_out_full[D_MODEL] = {};
    int8_t ffn1_weights_full[D_MODEL * D_MODEL] = {};
    int32_t ffn1_bias_full[D_MODEL] = {};
    int16_t ffn_act_out[D_FFN] = {};
    int8_t ffn2_weights[D_FFN * D_TILE_W2] = {};
    int32_t ffn2_out[D_TILE_W2] = {};
    int8_t ffn2_weights_full[D_FFN * D_FFN] = {};
    ...
    for (int i = 0; i < D_MODEL * D_MODEL; ++i) {
        ffn1_weights_full[i] = static_cast<int8_t>(3);
    }
    for (int i = 0; i < D_MODEL; ++i) {
        ffn1_bias_full[i] = 5;
    }
    for (int i = 0; i < D_FFN * D_FFN; ++i) {
        ffn2_weights_full[i] = static_cast<int8_t>(2);
    }
    for (int i = 0; i < D_FFN; ++i) {
        ffn2_bias_full[i] = 4;
    }
            ...
            if (mem_timer == 0) {
                ...
                if (mem_pending == MemPending::READ) {
                    ...
                    switch (mem_op_code) {
                        case ComputeOp::CMP_OUT_PROJ: {
                            ...
                            for (int i = 0; i < D_MODEL * D_TILE_WO; ++i) {
                                compute_buf::write_i8(
                                    in_buf,
                                    compute_buf::INOutProjLayout::W + i,
                                    valueB_in[i]);
                            }
                        ...
                        case ComputeOp::CMP_RESID1: {
                            for (int i = 0; i < D_MODEL; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INResidLayout::X + i, rq1_out[i]);
                                compute_buf::write_i8(in_buf, compute_buf::INResidLayout::R + i, valueA_mem[i]);
                            }
                            break;
                        }
                        case ComputeOp::CMP_RESID2: {
                            for (int i = 0; i < D_MODEL; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INResidLayout::X + i, rq3_out[i]);
                                compute_buf::write_i8(in_buf, compute_buf::INResidLayout::R + i, valueA_mem[i]);
                            }
                            break;
                        }
                        case ComputeOp::CMP_LN0: {
                            for (int i = 0; i < D_MODEL; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INLayerNormLayout::X + i, valueA_mem[i]);
                                compute_buf::write_i32(in_buf, compute_buf::INLayerNormLayout::GAMMA + (i * 4), 1);
                        ...
                        case ComputeOp::CMP_LN1: {
                            for (int i = 0; i < D_MODEL; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INLayerNormLayout::X + i, resid1_out[i]);
                                compute_buf::write_i32(in_buf, compute_buf::INLayerNormLayout::GAMMA + (i * 4), 1);
                                compute_buf::write_i32(in_buf, compute_buf::INLayerNormLayout::BETA + (i * 4), 0);
                            }
                            compute_buf::write_i32(in_buf, compute_buf::INLayerNormLayout::EPS, 1);
                        ...
                        case ComputeOp::CMP_FFN_W1: {
                            ...
                            for (int i = 0; i < D_MODEL; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INFfnW1Layout::X + i, head_requant_out[i]);
                            }
                            for (int i = 0; i < D_MODEL * D_TILE_W1; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INFfnW1Layout::W + i, ffn1_weights[i]);
                            }
                        ...
                        case ComputeOp::CMP_FFN_ACT: {
                            for (int i = 0; i < D_FFN; ++i) {
                                ...
                                compute_buf::write_i16(
                                    in_buf,
                                    compute_buf::INFfnActLayout::X + (i * 2),
                                    v);
                            }
                            break;
                        }
                        case ComputeOp::CMP_FFN_W2: {
                            ...
                            for (int i = 0; i < D_FFN; ++i) {
                                compute_buf::write_i16(in_buf, compute_buf::INFfnW2Layout::X + (i * 2), ffn_act_out[i]);
                            }
                            for (int i = 0; i < D_FFN * D_TILE_W2; ++i) {
                                compute_buf::write_i8(in_buf, compute_buf::INFfnW2Layout::W + i, ffn2_weights[i]);
                            }
                    ...
                    switch (mem_op_code) {
                        ...
                        case ComputeOp::CMP_FFN_ACT: {
                            for (int i = 0; i < D_FFN; ++i) {
                                ffn_act_out[i] = compute_buf::read_i16(out_buf, compute_buf::INFfnActLayout::X + (i * 2));
                            }
                            break;
                        }
                        case ComputeOp::CMP_FFN_W2: {
