...
static bool init_tb_logs() {
    ...
    char log_dir[512];
    std::snprintf(log_dir, sizeof(log_dir), "%s/%s", log_root.c_str(), TOP_DEBUG_TB_LOG_SUBDIR);
...
static inline int8_t stim_w8(int layer, int idx, int salt) {
    const LayerInputProfile p = layer_profile(layer);
    int v = 0;
    ...
    case ComputeOp::CMP_OUT_PROJ: {
        ...
        for (int i = 0; i < D_MODEL; ++i) {
	        ...
	        for (int i = 0; i < D_MODEL * D_TILE_WO; ++i) {
	            std::printf(" %d", static_cast<int>(compute_buf::read_i8(in_buf, compute_buf::INOutProjLayout::W + i)));
	        }
        std::printf("\n  B:");
    ...
    case ComputeOp::CMP_FFN_W1: {
        ...
        for (int i = 0; i < D_MODEL; ++i) {
	        ...
	        for (int i = 0; i < D_MODEL * D_TILE_W1; ++i) {
	            std::printf(" %d", static_cast<int>(compute_buf::read_i8(in_buf, compute_buf::INFfnW1Layout::W + i)));
	        }
        std::printf("\n  B:");
    ...
    case ComputeOp::CMP_FFN_W2: {
        ...
        for (int i = 0; i < D_FFN; ++i) {
	        ...
	        for (int i = 0; i < D_FFN * D_TILE_W2; ++i) {
	            std::printf(" %d", static_cast<int>(compute_buf::read_i8(in_buf, compute_buf::INFfnW2Layout::W + i)));
	        }
        std::printf("\n  B:");
    ...
    case ComputeOp::CMP_LOGITS: {
        ...
        for (int i = 0; i < D_MODEL; ++i) {
	        ...
	        for (int i = 0; i < D_MODEL * D_TILE_LOGIT; ++i) {
	            std::printf(" %d",
	                        static_cast<int>(compute_buf::read_i8(in_buf, compute_buf::INLogitsLayout::W + i)));
	        }
        std::printf("\n");
        break;
    }
    ...
    case ComputeOp::CMP_V: {
        ...
        for (int i = 0; i < D_MODEL; ++i) {
	        ...
	        for (int i = 0; i < D_MODEL * D_HEAD_TILE_QKV; ++i) {
	            std::printf(" %d", static_cast<int>(compute_buf::read_i8(in_buf, head_buf::INQkvLayout::W + i)));
	        }
        std::printf("\n");
        break;
    }
...
static int8_t g_wq[HEADS_PARALLEL][D_MODEL * D_HEADS] = {};
static int8_t g_wk[HEADS_PARALLEL][D_MODEL * D_HEADS] = {};
static int8_t g_wv[HEADS_PARALLEL][D_MODEL * D_HEADS] = {};
...
static void fill_head_lane_inputs(int lane, int layer) {
    ...
    for (int i = 0; i < D_MODEL * D_HEADS; ++i) {
        int q_w = static_cast<int>(stim_w8(layer, i, 41 + lane));
        if (q_w > 3) {
            q_w = 3;
        } else if (q_w < -3) {
            q_w = -3;
        }
        g_wq[lane][i] = static_cast<int8_t>(q_w);
        g_wk[lane][i] = stim_w8(layer, i, 51 + lane);
        g_wv[lane][i] = stim_w8(layer, i, 61 + lane);
    }
    ...
    // +A*+W, +A*-W, -A*+W, -A*-W.
    force_i8_i8_mm_corners(g_k_act[lane], D_MODEL, g_wk[lane], D_HEADS, D_MODEL);
    force_i8_i8_mm_corners(g_v_act[lane], D_MODEL, g_wv[lane], D_HEADS, D_MODEL);
...
static void build_head_in_buf(int lane, int layer, ComputeOp op, uint8_t head_in_buf[HEADS_PARALLEL][head_buf::IN_BUF_BYTES]) {
    ...
    case ComputeOp::CMP_V: {
        const int8_t *act = (op == ComputeOp::CMP_Q) ? g_q_act[lane]
                              ...
                              : g_v_act[lane];
        const int8_t *w = (op == ComputeOp::CMP_Q) ? g_wq[lane]
        ...
        for (int i = 0; i < D_MODEL * D_HEADS; ++i) {
            compute_buf::write_i8(buf, head_buf::INQkvLayout::W + i, w[i]);
        }
        break;
    }
    case ComputeOp::CMP_K_REQUANT:
...
static uint64_t compute_wl_address(uint32_t instr, const ControlMemSpace &ctrl) {
    ...
    case DMASEL_WO:
	    ...
	    case DMASEL_W1:
	        if (f.tile < 0) return 0;
	        return static_cast<uint64_t>(ctrl.w1_offset) + layer_u * static_cast<uint64_t>(STRIDE_W1_GATE_LAYER) +
