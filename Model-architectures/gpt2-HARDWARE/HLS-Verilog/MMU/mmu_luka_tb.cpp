...
int main() {
    ...
    for (int t = 0; t < NUM_QKV_HEAD_TILES; ++t) {
        add_dma(DMASEL_WQ, 0, 0, t, "Headed preload WQ for head0");
        add_cmp(CMP_Q, ComputeReqType::READ, 0, 0, t, "Consume WQ via CMP_Q READ (head0)");
        add_cmp(CMP_Q, ComputeReqType::WRITE, 0, 0, t, "Seed Q_OUT for ATT_SCORES consume path (head0)");
    }
    for (int t = 0; t < NUM_QKV_HEAD_TILES; ++t) {
        add_dma(DMASEL_WK, 0, 1, t, "Headed preload WK for head1");
        add_cmp(CMP_K, ComputeReqType::READ, 0, 1, t, "Consume WK via CMP_K READ (head1)");
    }
    for (int t = 0; t < NUM_QKV_HEAD_TILES; ++t) {
        add_dma(DMASEL_WV, 0, 0, t, "Headed preload WV for head0");
        add_cmp(CMP_V, ComputeReqType::READ, 0, 0, t, "Consume WV via CMP_V READ (head0)");
    }
    for (int t = 0; t < NUM_ATT_CTX_BLOCKS; ++t) {
        add_dma(DMASEL_CTX_K, 0, 0, t, "Headed preload K cache for head0");
        add_cmp(CMP_ATT_SCORES, ComputeReqType::READ, 0, 0, t, "Consume CTX_K via CMP_ATT_SCORES READ (head0)");
    }
    for (int t = 0; t < NUM_ATT_VALUE_HEAD_TILES; ++t) {
        add_dma(DMASEL_CTX_V, 0, 1, t, "Headed preload V cache for head1");
    }
    add_cmp(CMP_SOFTMAX, ComputeReqType::WRITE, 0, 1, -1, "Seed SOFTMAX_OUT for ATT_VALUE consume path (head1)");
    for (int t = 0; t < NUM_ATT_VALUE_HEAD_TILES; ++t) {
        add_cmp(CMP_ATT_VALUE, ComputeReqType::READ, 0, 1, t, "Consume CTX_V via CMP_ATT_VALUE READ (head1)");
    }

    // 4) Headed parallel requests (DMA + compute in same cycle).
    add_parallel_head(DMASEL_CTX_K, 0, 0, 0, CMP_Q, ComputeReqType::WRITE, 0, 1, 0,
                      "Parallel #1: DMA CTX_K(h0) + CMP_Q WRITE(h1)");
    add_parallel_head(DMASEL_CTX_V, 0, 1, 0, CMP_K, ComputeReqType::WRITE, 0, 0, 0,
                      "Parallel #2: DMA CTX_V(h1) + CMP_K WRITE(h0)");
    add_parallel_head(DMASEL_WQ, 0, 1, 0, CMP_V, ComputeReqType::WRITE, 0, 1, 0,
    ...
    for (int t = 1; t < NUM_QKV_HEAD_TILES; ++t) {
        add_cmp(CMP_Q, ComputeReqType::WRITE, 0, 0, t, "Seed Q_OUT(h0) pt2");
        add_cmp(CMP_K, ComputeReqType::WRITE, 0, 0, t, "Seed K_OUT(h0) pt2");
        add_cmp(CMP_V, ComputeReqType::WRITE, 0, 1, t, "Seed V_OUT(h1) pt2");
    }
    for (int t = 1; t < NUM_ATT_CTX_BLOCKS; ++t) {
        add_cmp(CMP_ATT_SCORES, ComputeReqType::READ, 0, 0, t, "Consume CTX_K(h0) after parallel");
    }
    add_cmp(CMP_SOFTMAX, ComputeReqType::WRITE, 0, 1, -1, "Seed SOFTMAX_OUT(h1) for CTX_V consume after parallel");
    for (int t = 1; t < NUM_ATT_VALUE_HEAD_TILES; ++t) {
        add_cmp(CMP_ATT_VALUE, ComputeReqType::READ, 0, 1, t, "Consume CTX_V(h1) after parallel");
    }
    for (int t = 1; t < NUM_QKV_HEAD_TILES; ++t) {
        add_cmp(CMP_Q, ComputeReqType::READ, 0, 1, t, "Consume WQ(h1) after parallel");
    }
