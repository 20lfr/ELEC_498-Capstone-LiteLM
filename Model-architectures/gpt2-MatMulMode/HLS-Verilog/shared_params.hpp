...
#define FULL_MODEL_TEST
    ...
    constexpr int D_TILE_SHARED        = 16;  // 48 tiles over D_MODEL=768
    constexpr int D_TILE_WO            = D_TILE_SHARED;  // 48 tiles over D_MODEL=768
    constexpr int D_TILE_W1            = D_TILE_SHARED;  // 192 tiles over D_FFN=3072
    constexpr int D_TILE_W2            = 8;   // 96 tiles over D_MODEL=768; increased from 4 to unlock MAC_OP_TO_BUF_OUT_UNROLL=8
    constexpr int D_TILE_LOGIT         = D_TILE_SHARED;  // 3142 tiles over D_VOCAB=50257 (ceil); must be divisible by MAC_OP_TO_BUF_OUT_UNROLL_TARGET
    ...
    constexpr int NUM_HEADS     = D_MODEL / D_HEADS;
    // Tile widths (primary — tile counts derived below)
    // NOTE: small model has unequal tile sizes; MATMUL_SHARED is not valid here
    constexpr int D_TILE_SHARED        = 4;  // = D_TILE_WO; WO/W1/LOGIT not equal in small model
    constexpr int D_TILE_WO            = D_TILE_SHARED;
...
static_assert(D_TILE_WO == D_TILE_W1 && D_TILE_W1 == D_TILE_LOGIT,
    "MATMUL_SHARED requires D_TILE_WO == D_TILE_W1 == D_TILE_LOGIT (set D_TILE_SHARED)");
