...
constexpr const char *DEFAULT_MODEL_BIN_PATH =
    "/home/luka/Scripting/model/gpt2_weights_int8.bin";
...
inline bool copy_i8_rows(std::ifstream &in,
...
inline bool load_prebuilt_ddr_image(const std::string &image_path,
    ...
    if (static_cast<uint64_t>(file_size) < total_bytes) {
        std::fprintf(stderr,
                     "ERROR: Generated DDR image '%s' too small "
    ...
    // The generated DDR image is already in packed, byte-addressable HLS layout.
    // Load it verbatim into the local DMA staging buffer.
    in.seekg(0, std::ios::beg);
    std::vector<uint8_t> buf(static_cast<size_t>(total_bytes), 0);
    in.read(reinterpret_cast<char *>(buf.data()),
            static_cast<std::streamsize>(buf.size()));
    if (static_cast<size_t>(in.gcount()) != buf.size()) {
        std::fprintf(stderr,
                     "ERROR: Failed to read generated DDR image '%s' "
                     "(wanted=%zu got=%lld)\n",
                     image_path.c_str(),
                     buf.size(),
                     static_cast<long long>(in.gcount()));
    ...
    return write_axi_bytes(ddr_mem, word_count, 0, buf.data(), buf.size(),
                           "DDR_IMAGE");
}
...
inline bool load_compact_weights_image(const std::string &image_path,
    ...
    const size_t compact_hidden_bytes = packed_i8_bytes(D_MODEL);
    const size_t compact_ffn_bytes = packed_i8_bytes(D_FFN);
        ...
        for (int head = 0; head < NUM_HEADS; ++head) {
            const uint64_t src_row0 =
                ...
                static_cast<uint64_t>(head * MODEL_HEAD_DIMENSTION) *
                    static_cast<uint64_t>(MODEL_HIDDEN_SIZE);
    ...
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        for (int tile = 0; tile < NUM_WO_TILES; ++tile) {
            ...
            if (!copy_i8_rows(in, ddr_mem, word_count, "WO", src_off,
                              static_cast<uint64_t>(MODEL_HIDDEN_SIZE),
                              D_TILE_WO,
    ...
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        for (int tile = 0; tile < NUM_W1_TILES; ++tile) {
            const uint64_t w1_src_off =
                W1_OFF +
                static_cast<uint64_t>(layer) * STRIDE_W1_GATE_LAYER +
                static_cast<uint64_t>(tile * D_TILE_W1) *
                    static_cast<uint64_t>(MODEL_HIDDEN_SIZE);
            const uint64_t w1_dst_off =
                static_cast<uint64_t>(W1_OFF) +
                static_cast<uint64_t>(layer) * STRIDE_W1_GATE_LAYER +
                static_cast<uint64_t>(tile) * STRIDE_W1_TILE;
            if (!copy_i8_rows(in, ddr_mem, word_count, "W1", w1_src_off,
                              static_cast<uint64_t>(MODEL_HIDDEN_SIZE),
                              D_TILE_W1,
                              compact_hidden_bytes, w1_dst_off)) {
                return false;
            }
        }
    }
        ...
        for (int tile = 0; tile < NUM_W2_TILES; ++tile) {
            const uint64_t src_off =
                ...
                static_cast<uint64_t>(tile * D_TILE_W2) *
                    static_cast<uint64_t>(MODEL_INTERMEDIATE_SIZE);
            const uint64_t dst_off =
                static_cast<uint64_t>(W2_OFF) +
                static_cast<uint64_t>(layer) * STRIDE_W2_LAYER +
                static_cast<uint64_t>(tile) * STRIDE_W2_TILE;
            if (!copy_i8_rows(in, ddr_mem, word_count, "W2", src_off,
                              static_cast<uint64_t>(MODEL_INTERMEDIATE_SIZE),
                              D_TILE_W2,
    ...
    for (int row = 0; row < D_VOCAB; ++row) {
        ...
        if (!copy_i8_rows(in, ddr_mem, word_count, "WLOGIT", src_off,
                          static_cast<uint64_t>(MODEL_HIDDEN_SIZE), 1,
